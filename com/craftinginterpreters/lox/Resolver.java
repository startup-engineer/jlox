...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
    ...
    private class VariableState {
        ...
        VariableState(Token token, boolean isDefined, boolean isUsed) {
            this.token = token;
    ...
    private void endScope() {
        ...
        for (Map.Entry<String, VariableState> entry : scope.entrySet()) {
            if (!entry.getValue().isUsed) {
                Lox.error(entry.getValue().token, "Variable '" + entry.getKey() + "' is defined but not used.");
            }
        }
    }
    ...
    private void declare(Token name) {
        ...
        scope.put(name.lexeme, new VariableState(name, false, false));
    }
